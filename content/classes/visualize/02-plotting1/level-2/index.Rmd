...
We are going use what we learnt in [Level 3 of the Introduction](../../01-introduction/level-3/) to read the data in using the `read_csv()` function. First, we are going to read the single profile file. This data was recorded off the coast of Antarctica at the location shown by the red cross below.
...
The object `profile` is now a data frame containing the data in `profiles.csv`. It should be available to you in the environment panel where you can click on it to see it in spreadsheet format.
...
* The columns have relatively self-explanatory headings, but we always need the metadata that tells us what each is specifically. In out case:
    ...
    + sigma: Potential Density (kgm^-^3) - thedensity of the water as calculated from the salinity, pressure and potential temperature.
...
*We've lost something in the ability for us to directly interpret what is being assigned to what, but we're saving our fingers in the long run. It's up to you which makes more sense to you at this point. There's issue at all in staying verbose.*
