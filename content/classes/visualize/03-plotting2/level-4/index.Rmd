...
This level will introduce to a couple of advanced tools that will be helpful in making plots. Specifically, how to make multiple panels and how to customize your theme.
...
We're going to use [hourly recorded surface data](C282_hourly_data.csv) from the same cruise we have looked at previously from Woods Hole, MA to Greneda. The columns we are going to focus on are:
...
```{r}
df %>% select(c(dttm, temp, sal, wind_sp))

df %>% 
  pivot_longer(c(temp, sal, wind_sp)) %>%
...
Could certainly be seen as much easier.
...
We'll now add some modificiations. *NOTE: Not all of the below ideas will likely constitute good design choices, but I wanted to give you examples of the full range of possibilities.* 
...
Try this yourself using in the exercise below.
